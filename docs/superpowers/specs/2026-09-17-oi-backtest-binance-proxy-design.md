...
## 7. 부록: hold-to-bracket 재설계 (2026-09-17 승인)

oneshot/그리드 프로브 결과(§6 리스크 확인): 두 전략 모두 기대값 = 라운드트립 수수료(-0.27 USDT/trade)로 파라미터 강건한 음의 엣지. 원인은 반대쪽 BB 밴드 TP가 도달 불가능하고 세션 종료 청산이 브레이크이버스 강제 전환하는 구조.

재설계 (config-gated, 기본값은 2026-09-16 스펙 동작 보존):

| 항목 | 결정 |
|---|---|
| `tp_mode` | `"band"`(기본, 기존) / `"atr"`: TP = 진입가 ± `tp_atr_mult` × 진입 시점 ATR |
| `tp_atr_mult` | 2.0 (기본). 그리드 축 [1.5, 2.5, 4.0] |
| `hold_across_sessions` | `False`(기본) / `True`: 세션 종료·주말 강제 청산 스킵, 브래킷 체결까지 보유. 진입은 세션 내에서만 (진입 메커니즘 동일) |
| SL | 기존 `atr_mult` 그대로, 그리드 축 [2.0, 3.0, 4.0] |
| 펀딩 | 평가에 trade-level 반영: 트레이드별 (진입~청산) 구간 OKX 실측 펀딩비 귀속. IS 선택·OOS 평가·MC 전부 funded 수치 사용 |
| 평가 흐름 | 2023 1년 18셀 프로브 → 양의 기대값 셀 존재 시 WF 전면 실행 |
| 리스크 | 주말/격일 보유 갭 슬리피지는 백테스트 모델이 과소평가 가능 — assumptions에 명시 |
