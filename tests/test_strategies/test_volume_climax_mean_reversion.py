...
def make_strategy(**overrides) -> VolumeClimaxMeanReversion:
    values = {
    ...
    return VolumeClimaxMeanReversion(VolumeClimaxMeanReversionConfig(**values))


def test_vc_defaults_match_spec():
    ...
    assert strategy.config.vol_spike_lookback == 20
    assert strategy.config.vol_spike_threshold == 2.0
    assert strategy.config.max_trades_per_session == 3


def test_vc_long_requires_breach_and_volume_spike():
    strategy = make_strategy()
    strategy._test_feed_volumes([1.0] * 20 + [3.0])
    assert strategy._test_start_setup(side=1, close=98.0, lower=99.0)


def test_vc_short_requires_breach_and_volume_spike():
    strategy = make_strategy()
    strategy._test_feed_volumes([1.0] * 20 + [3.0])
    assert strategy._test_start_setup(side=-1, close=102.0, upper=101.0)


def test_vc_rejects_breach_without_spike():
    strategy = make_strategy()
    strategy._test_feed_volumes([1.0] * 20 + [1.0])
    assert not strategy._test_start_setup(side=1, close=98.0, lower=99.0)


def test_vc_rejects_insufficient_volume_history():
    strategy = make_strategy()
    strategy._test_feed_volumes([1.0] * 3)
    assert not strategy._test_start_setup(side=1, close=98.0, lower=99.0)
    assert strategy._pending_setup is None


def test_vc_first_reentry_submits_once_and_expires_after_five_bars():
    strategy = make_strategy()
    strategy._test_feed_volumes([1.0] * 20 + [3.0])
    assert strategy._test_start_setup(side=1, close=98.0, lower=99.0)
    ...
    expired._test_feed_volumes([1.0] * 20 + [3.0])
...
def test_vc_short_reentry_submits_sell_order(monkeypatch):
    strategy = make_strategy()
    strategy._test_feed_volumes([1.0] * 20 + [3.0])
    assert strategy._test_start_setup(side=-1, close=102.0, upper=101.0)
    submitted = []
...
def test_vc_session_uses_new_york_calendar_and_dst():
    strategy = make_strategy()
...
def test_vc_rejects_fourth_filled_entry():
    strategy = make_strategy()
...
def test_vc_at_close_cancels_setup_and_closes_positions(monkeypatch):
    strategy = make_strategy()
...
def test_vc_fill_submits_reduce_only_stop_and_target(monkeypatch):
    strategy = make_strategy()
    submitted = []
...
def test_vc_partial_entry_fills_resize_one_exit_pair(monkeypatch):
    strategy = make_strategy()
    submitted = []
...
def test_vc_on_bar_breach_without_spike_never_sets_up(monkeypatch):
    strategy = make_strategy()
    submitted = []
    _install_order_fakes(monkeypatch, strategy)
    strategy._instrument = lambda: SimpleNamespace(
...
def test_vc_on_bar_breach_with_spike_then_reentry_submits_entry(monkeypatch):
    strategy = make_strategy()
    submitted = []
    _install_order_fakes(monkeypatch, strategy)
    strategy._instrument = lambda: SimpleNamespace(
