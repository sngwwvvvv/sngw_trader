...
def make_strategy(**overrides) -> BbMeanReversion:
    values = {
    ...
    return BbMeanReversion(BbMeanReversionConfig(**values))


def test_bb_defaults_match_spec():
    ...
    assert strategy.config.session_timezone == "America/New_York"
    assert strategy.config.session_start == "09:30"
    assert strategy.config.session_end == "16:00"
    assert strategy.config.max_trades_per_session == 3


def test_bb_long_requires_lower_breach():
    strategy = make_strategy()
    assert strategy._test_start_setup(side=1, close=98.0, lower=99.0)


def test_bb_short_requires_upper_breach():
    strategy = make_strategy()
    assert strategy._test_start_setup(side=-1, close=102.0, upper=101.0)


def test_bb_no_breach_rejected():
    strategy = make_strategy()
    with pytest.raises(ValueError):
        strategy._test_start_setup(side=1, close=100.0, lower=99.0)
    with pytest.raises(ValueError):
        strategy._test_start_setup(side=-1, close=100.0, upper=101.0)
    assert strategy._pending_setup is None


def test_bb_first_reentry_submits_once_and_expires_after_five_bars():
    strategy = make_strategy()
    assert strategy._test_start_setup(side=1, close=98.0, lower=99.0)
    assert strategy._test_reentry(
        side=1, close=100.0, lower=99.0, upper=101.0
    )
...
def test_bb_short_reentry_submits_sell_order(monkeypatch):
    strategy = make_strategy()
    assert strategy._test_start_setup(side=-1, close=102.0, upper=101.0)
    submitted = []
...
def test_bb_session_uses_new_york_calendar_and_dst():
    strategy = make_strategy()
...
def test_bb_rejects_fourth_filled_entry():
    strategy = make_strategy()
...
def test_bb_at_close_cancels_setup_and_closes_positions(monkeypatch):
    strategy = make_strategy()
...
def test_bb_fill_submits_reduce_only_stop_and_target(monkeypatch):
    strategy = make_strategy()
    submitted = []
...
def test_bb_partial_entry_fills_resize_one_exit_pair(monkeypatch):
    strategy = make_strategy()
    submitted = []
...
def test_bb_on_bar_warms_indicators_outside_session(monkeypatch):
    strategy = make_strategy()
    calls = []
    monkeypatch.setattr(strategy, "cancel_all_orders", lambda instrument_id: calls.append("cancel"))
    monkeypatch.setattr(strategy, "close_all_positions", lambda instrument_id: calls.append("close"))
...
def test_bb_on_bar_breach_then_reentry_submits_entry(monkeypatch):
    strategy = make_strategy()
    submitted = []
    _install_order_fakes(monkeypatch, strategy)
    strategy._instrument = lambda: SimpleNamespace(
