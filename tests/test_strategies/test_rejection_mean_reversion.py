...
from sngw_trader.strategies.rejection_mean_reversion import (
    RejectionMeanReversion,
    RejectionMeanReversionConfig,
)
...
def make_strategy(**overrides) -> RejectionMeanReversion:
    values = {
    ...
    return RejectionMeanReversion(RejectionMeanReversionConfig(**values))


def test_r_defaults_match_spec():
    ...
    assert strategy.config.rejection_threshold == 0.66
    assert strategy.config.max_trades_per_session == 3


def test_r_long_requires_breach_and_rejection():
    strategy = make_strategy()
    assert strategy._test_start_setup(
        side=1, close=99.5, high=100.0, low=90.0, lower=99.6
    )


def test_r_short_requires_breach_and_rejection():
    strategy = make_strategy()
    assert strategy._test_start_setup(
        side=-1, close=100.5, high=110.0, low=100.0, upper=100.4
    )


def test_r_rejects_breach_without_rejection():
    strategy = make_strategy()
    assert not strategy._test_start_setup(
        side=1, close=91.0, high=100.0, low=90.0, lower=99.6
    )


def test_r_rejects_degenerate_range():
    strategy = make_strategy()
    assert not strategy._test_start_setup(
        side=1, close=100.0, high=100.0, low=100.0, lower=100.5
    )
    assert strategy._pending_setup is None


def test_r_first_reentry_submits_once_and_expires_after_five_bars():
    strategy = make_strategy()
    assert strategy._test_start_setup(
        side=1, close=99.5, high=100.0, low=90.0, lower=99.6
    )
    assert strategy._test_reentry(side=1, close=100.0, lower=99.6, upper=101.0)

    expired = make_strategy()
    assert expired._test_start_setup(
        side=1, close=99.5, high=100.0, low=90.0, lower=99.6
    )
    for _ in range(5):
        expired._test_advance_pending(close=99.0, lower=99.6, upper=101.0)
    assert expired._pending_setup is None


def test_r_short_reentry_submits_sell_order(monkeypatch):
    strategy = make_strategy()
    assert strategy._test_start_setup(
        side=-1, close=100.5, high=110.0, low=100.0, upper=100.4
    )
    ...
    assert strategy._process_pending(
        close=100.0, lower=99.0, upper=100.4, atr=1.0, target=99.0, submit=True
    )

    assert submitted[0].order_side == OrderSide.SELL


def test_r_session_uses_new_york_calendar_and_dst():
    strategy = make_strategy()
...
def test_r_rejects_fourth_filled_entry():
    strategy = make_strategy()
...
def test_r_at_close_cancels_setup_and_closes_positions(monkeypatch):
    strategy = make_strategy()
...
def test_r_fill_submits_reduce_only_stop_and_target(monkeypatch):
    strategy = make_strategy()
    submitted = []
...
def test_r_partial_entry_fills_resize_one_exit_pair(monkeypatch):
    strategy = make_strategy()
    submitted = []
    _install_order_fakes(monkeypatch, strategy)
    strategy._captured_atr = 2.0
...
class _Bar:
    ...
    def __init__(self, ts_event: int, close: float, high: float, low: float):
...
def test_r_on_bar_breach_rejection_then_reentry_submits_entry(monkeypatch):
    strategy = make_strategy()
    submitted = []
    _install_order_fakes(monkeypatch, strategy)
    strategy._instrument = lambda: SimpleNamespace(
    ...
    for index in range(21):
        strategy.on_bar(_Bar(start + index * step, close=100.0, high=100.5, low=99.5))
    assert strategy._bb.initialized

    strategy.on_bar(_Bar(start + 21 * step, close=99.5, high=99.6, low=98.0))
    assert strategy._pending_setup is not None
    assert strategy._pending_setup.side == 1

    strategy.on_bar(_Bar(start + 22 * step, close=100.5, high=101.0, low=100.0))
    assert [order.kind for order in submitted] == ["entry"]
    assert submitted[0].order_side == OrderSide.BUY


def test_r_on_bar_breach_without_rejection_never_sets_up(monkeypatch):
    strategy = make_strategy()
    submitted = []
    _install_order_fakes(monkeypatch, strategy)
    strategy._instrument = lambda: SimpleNamespace(
    ...
    strategy.on_bar(_Bar(start + 21 * step, close=98.5, high=99.6, low=98.4))
    assert strategy._pending_setup is None
...
def test_hold_across_sessions_skips_force_flat_at_close(monkeypatch):
    ...
    strategy.on_bar(_Bar(close_ts, 100.0, 101.0, 99.0))
