...
def test_b_atr_tp_long_target_is_fill_plus_mult_times_atr(monkeypatch):
...
def test_b_atr_tp_short_target_is_fill_minus_mult_times_atr(monkeypatch):
    strategy = make_strategy(tp_mode="atr", tp_atr_mult=3.0)
    submitted = []
    strategy._captured_atr = 2.0
    strategy._captured_target = 105.0
...
def test_b_rejects_invalid_tp_mode():
    with pytest.raises(ValueError, match="tp_mode"):
        make_strategy(tp_mode="invalid")


def test_b_hold_across_sessions_skips_force_flat_at_close(monkeypatch):
