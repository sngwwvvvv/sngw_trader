"""Volume-fade mean-reversion (H2).

Bollinger-band breach plus fading volume on the breach bar (volume below
its recent mean), then entry on re-entry cross. The fade read: the push
beyond the band has no fresh money behind it, so it reverses.
"""
...
from sngw_trader.indicators.volume_mean_reversion import is_volume_fading
...
class VolumeFadeMeanReversionConfig(StrategyConfig, frozen=True, kw_only=True):
    ...
    atr_mult: float = 2.5
    reentry_bars: int = 5
    vol_fade_lookback: int = 20
    vol_fade_threshold: float = 0.5
...
class VolumeFadeMeanReversion(NavFractionMixin, Strategy):
    """Price-band breach plus fading volume, then re-entry cross."""

    def __init__(self, config: VolumeFadeMeanReversionConfig) -> None:
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if not self._can_submit_entry(local_dt):
            return
        if not is_volume_fading(
            self._volumes,
            self.config.vol_fade_lookback,
            self.config.vol_fade_threshold,
        ):
            return
        if close < lower:
