"""Volume-climax mean-reversion (H1).

Bollinger-band breach plus an abnormal volume spike on the breach bar,
then entry on re-entry cross. The volume spike is the capitulation read:
a liquidation/coverage flush that exhausts the extension.
"""
...
class VolumeClimaxMeanReversionConfig(StrategyConfig, frozen=True, kw_only=True):
...
class VolumeClimaxMeanReversion(NavFractionMixin, Strategy):
    """Price-band breach plus a volume climax, then re-entry cross."""

    def __init__(self, config: VolumeClimaxMeanReversionConfig) -> None:
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if not is_volume_spike(
            self._volumes,
            self.config.vol_spike_lookback,
            self.config.vol_spike_threshold,
        ):
            return
        if close < lower:
