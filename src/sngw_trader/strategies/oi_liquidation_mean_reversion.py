"""OI liquidation contraction mean-reversion Strategy B."""
...
class OiLiquidationMeanReversionConfig(StrategyConfig, frozen=True, kw_only=True):
    ...
    oi_decrease_threshold: float = 0.0
...
class OiLiquidationMeanReversion(NavFractionMixin, Strategy):
    """Price-band excursion plus lagged OI decrease."""

    def __init__(self, config: OiLiquidationMeanReversionConfig) -> None:
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if close < lower and is_oi_decreasing(
            values,
            self.config.oi_lookback_bars,
            self.config.oi_decrease_threshold,
        ):
            self._pending_setup = _PendingSetup(1, 0)
        elif close > upper and is_oi_decreasing(
            values,
            self.config.oi_lookback_bars,
            self.config.oi_decrease_threshold,
        ):
    ...
    def _test_start_setup(
        ...
        *,
        ...
        values = [value for _, value in self._oi]
        if not is_oi_decreasing(
            values,
            self.config.oi_lookback_bars,
            self.config.oi_decrease_threshold,
        ):
