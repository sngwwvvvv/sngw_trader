"""Wick-rejection mean-reversion (H3).

Bollinger-band breach with a rejection wick on the breach bar (close
retraced back toward the extreme of its range), then entry on re-entry
cross. Volume is deliberately absent; volume confirmation is H4's job.
"""
...
from sngw_trader.indicators.volume_mean_reversion import is_rejection
...
class RejectionMeanReversionConfig(StrategyConfig, frozen=True, kw_only=True):
    ...
    atr_mult: float = 2.5
    reentry_bars: int = 5
    rejection_threshold: float = 0.66
...
class RejectionMeanReversion(NavFractionMixin, Strategy):
    """Price-band breach plus a rejection wick, then re-entry cross."""

    def __init__(self, config: RejectionMeanReversionConfig) -> None:
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if not self._can_submit_entry(local_dt):
            return
        if close < lower and is_rejection(
            1, high, low, close, self.config.rejection_threshold
        ):
            self._pending_setup = _PendingSetup(1, 0)
        elif close > upper and is_rejection(
            -1, high, low, close, self.config.rejection_threshold
        ):
            self._pending_setup = _PendingSetup(-1, 0)
    ...
    def _test_start_setup(
        ...
        *,
        ...
        if not is_rejection(side, high, low, close, self.config.rejection_threshold):
            return False
