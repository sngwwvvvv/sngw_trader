"""OI crowding rollover mean-reversion Strategy A."""
...
from nautilus_trader.model import Bar, BarType, ClientId, CustomData, DataType, InstrumentId
...
class OiCrowdingMeanReversionConfig(StrategyConfig, frozen=True, kw_only=True):
    ...
    oi_increase_threshold: float = 0.0
    oi_rollover_threshold: float = 0.0
...
class OiCrowdingMeanReversion(Strategy):
    ...
    def on_start(self) -> None:
        ...
        data_type = DataType(OpenInterestPoint)
        if self.config.oi_client_id is None:
            self.subscribe_data(data_type)
        else:
            self.subscribe_data(data_type, client_id=ClientId(self.config.oi_client_id))
    ...
    def on_data(self, data) -> None:
        ...
        if not isinstance(point, OpenInterestPoint):
            return
        if point.instrument_id != self.config.instrument_id:
            return
        item = (int(point.ts_event), float(point.open_interest))
        index = bisect_left(self._oi, item)
        if index < len(self._oi) and self._oi[index][0] == item[0]:
            self._oi[index] = item
        else:
            self._oi.insert(index, item)
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        local_dt = datetime.fromtimestamp(ts / 1_000_000_000, tz=timezone.utc).astimezone(self._tz)
        ...
        latest_oi = self._latest_oi_before(ts)
        ...
        if not self._can_submit_entry(local_dt) or latest_oi is None:
            return
        values = self._oi_values_before(ts)
        if close < lower and is_oi_increasing(
            values, self.config.oi_lookback_bars, self.config.oi_increase_threshold
        ):
            self._pending_setup = _PendingSetup(1, 0, latest_oi)
        elif close > upper and is_oi_increasing(
            values, self.config.oi_lookback_bars, self.config.oi_increase_threshold
        ):
            self._pending_setup = _PendingSetup(-1, 0, latest_oi)
    ...
    def on_order_canceled(self, event) -> None:
        if self._same_order_id(getattr(event, "client_order_id", None), self._entry_order_id):
            self._entry_order_active = False
            self._entry_order = None
            if self._signed_qty == 0 and self._force_flat_date is None:
                self._clear_entry_state()
        if self._same_order_id(getattr(event, "client_order_id", None), self._stop_order_id):
            self._stop_order = None
            self._stop_order_id = None
        if self._same_order_id(getattr(event, "client_order_id", None), self._target_order_id):
            self._target_order = None
            self._target_order_id = None
    ...
    def _process_pending(
        ...
        *,
        ...
        if setup is None:
            return False
        setup.age += 1
        prior_peak = setup.oi_peak
        if latest_oi is not None:
            setup.oi_peak = max(prior_peak, latest_oi)
        if not setup.reentry_seen and reentry_side(setup.side, close, lower, upper):
            setup.reentry_seen = True
            rollover = latest_oi is not None and has_oi_rollover(
                latest_oi, prior_peak, self.config.oi_rollover_threshold
            )
            if rollover:
                if submit:
    ...
    def _submit_bracket(self, *, fill_price: float, quantity: Decimal, side: int) -> None:
        ...
        if self._stop_order is not None and self._target_order is not None:
            ...
            return
        # OrderFactory.bracket requires sl_trigger_price before the entry fill.
        # This stop is defined from the actual fill price, so a post-fill pair is
        # the safe native representation for this strategy.
    ...
    def _latest_oi_before(self, ts: int) -> float | None:
        index = bisect_left(self._oi, (ts, float("-inf")))
        return None if index == 0 else self._oi[index - 1][1]

    def _oi_values_before(self, ts: int) -> list[float]:
        index = bisect_left(self._oi, (ts, float("-inf")))
        return [value for _, value in self._oi[:index]]
    ...
    def _test_feed_oi(self, values: list[float]) -> None:
        self._oi = [(index + 1, value) for index, value in enumerate(values)]

    def _test_start_setup(self, *, side: int, close: float, lower: float) -> None:
        if side == 1 and close >= lower:
            raise ValueError("long breach requires close < lower")
        if side not in (1, -1):
            raise ValueError("side must be 1 or -1")
        peak = max((value for _, value in self._oi), default=104.0)
        self._pending_setup = _PendingSetup(side, 0, peak)

    def _test_reentry(
        self, *, side: int, close: float, lower: float, upper: float, oi: float
    ) -> bool:
        ...
        self._oi.append((max((ts for ts, _ in self._oi), default=0) + 1, oi))
    ...
    def _test_advance_pending(
        self, *, close: float, lower: float, upper: float, oi: float
    ) -> None:
        if self._pending_setup is None:
            raise AssertionError("pending setup is required")
        self._test_reentry(
