"""MACD crossover comparison matrix: long-only vs long+short over IS/OOS segments.

Handoff 20260905-bt-macd-crossover-core. MACD(12,26,9) fixed — no parameter
optimization (handoff constraint). Base variants use unit sizing
(strategy doc is silent on sizing); state trigger and vol-target sizing are
sensitivity runs reported separately, never selected.
...
research/macd_crossover/results.json + a console table. No alpha here.
"""
...
SPEC = GridSpec(
    strategy_path="sngw_trader.strategies.macd_crossover:MacdCrossover",
    config_path="sngw_trader.strategies.macd_crossover:MacdCrossoverConfig",
    fixed={"trade_size": "0.02"},
    grid={},
)

# Equity evidence markets (FT30 / Spain / US stocks) are not reproducible:
# the catalog holds only BTC-USDT-SWAP.OKX 1m bars (2019-12-16 ~ 2025-12-30).
SEGMENTS = {
...
WARMUP_DAYS = 120  # macd 26+9 + vol half-life*3 (60) -> 120 safe
...
SENS_VARIANTS = {
    "long_only/state": {"allow_short": False, "trigger_mode": "state"},
    "long_short/state": {"allow_short": True, "trigger_mode": "state"},
    "long_only/vt": {"allow_short": False, "sizing_mode": "vol_target"},
    "long_short/vt": {"allow_short": True, "sizing_mode": "vol_target"},
}

OUT_DIR = Path("research/macd_crossover")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="also run state trigger + vol-target sizing (reported, never selected)",
    )
    ...
    print("\n=== MACD crossover long_only vs long_short (base cost) ===")
